...
//? for of문 (배열, 객체)
for (let item of list7) { // A of B: B의 A
    console.log(item);
}
...
//? 1. map 메서드(method) ***중요***
// 배열 내의 모든 요소 각각에 대해 제공된 함수를 호출한 결과를 모아 새로운 배열을 반환
let doubledNumbers = numbers.map((num)=> num * 2 ); 
console.log(doubledNumbers); // [2, 4, 6, 8, 10]


//? 2. filter 메서드
// 제공된 함수를 테스트를 통과하는 모든 요소를 모아 새로운 배열로 반환
let evenNumbers = numbers.filter((num) => num % 2 === 0);
console.log(evenNumbers); //[2, 4]


//? 3. reduce 메서드
// 배열의 각 요소에 대해 제공된 reducer 함수를 실행한 후 단일 출력값으로 줄임
// 누적값을 호출하는 메서드
